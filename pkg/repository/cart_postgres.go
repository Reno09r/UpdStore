...
func (r *StoreCartPostgres) Insert(input store.Cart, customerId int) (int, error) {
	...
	if purchaseId, err := r.CheckProductInCart(customerId, input.ID); purchaseId == 0 && err != nil {
		...
		insertToCartQuery := "INSERT INTO purchases (purchase_date, customer_id) VALUES (NOW(), $1) RETURNING purchase_id;"

		row := tx.QueryRow(insertToCartQuery, customerId)
...
func (r *StoreCartPostgres) Get(customerId int) ([]store.Cart, error) {
	...
	`, ProductsTable, ManufacturerTable, CatalogTable, PriceTable)
	err := r.db.Select(&cart, query, customerId)
	return cart, err
}

func (r *StoreCartPostgres) Delete(productId, customerId int) error {
	queryCheck := "SELECT purchase_id FROM purchase_items WHERE product_id = $1 AND purchase_id IN (SELECT purchase_id FROM purchases WHERE customer_id = $2)"
	var purchaseID int
